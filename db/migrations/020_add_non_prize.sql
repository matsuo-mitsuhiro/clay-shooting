-- 020: 賞典外（非公式参加）フラグ追加
-- members テーブルに is_non_prize 列を追加
-- v_results ビューを更新：賞典外は通常選手の後ろに並び、賞典外同士で順位付け

ALTER TABLE members ADD COLUMN IF NOT EXISTS is_non_prize BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN members.is_non_prize IS '賞典外フラグ（TRUE=賞典対象外、順位集計では別枠扱い）';
...
merged AS (
  SELECT d1.tournament_id, d1.member_code, d1.name, d1.belong,
         d1.class, d1.is_judge, d1.is_non_prize,
         d1.group1, d1.position, d2.group2
...
-- 有効選手のみで順位付与
-- 賞典外と通常選手を別パーティションでランキング
valid_ranked AS (
  SELECT tournament_id, member_code,
    RANK() OVER (
      PARTITION BY tournament_id, is_non_prize
      ORDER BY
        total DESC,
        COALESCE(r8, 0) DESC,
        COALESCE(r7, 0) DESC,
...
COMMENT ON VIEW v_results IS '成績集計ビュー（合計・平均・カウントバック順位・CB・FR・賞典外フラグ付き。賞典外は通常選手と別枠で順位計算）';
